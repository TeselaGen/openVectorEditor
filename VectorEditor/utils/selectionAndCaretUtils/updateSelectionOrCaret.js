...
export default function updateSelectionOrCaret({shiftHeld, sequenceLength, newRangeOrCaret, caretPosition, selectionLayer, updateSelectionLayer, updateCaret}) {
    var newCaret
    var newRange
    ...
    if (shiftHeld) {
        if (caretPosition > 0) {
            ...
            if (newCaret > -1) {
                ...
                //     ? updateSelectionLayer(caretPosition, newCaret)
                //     : updateSelectionLayer(caretPosition, newCaret)
            } else {
                simpleUpdate()
            }
            ...
            if (newCaret > -1) {
                ...
                if (selectionFullyContained) {
                    return updateSelectionLayer(newRange)
                }
                ...
                if (newRangeFullyContained) {
                    range1Shorter 
                        ? updateSelectionLayer(range1)
                        : updateSelectionLayer(range2)
                } else {
                    range1Shorter 
                        ? updateSelectionLayer(range2)
                        : updateSelectionLayer(range1)
