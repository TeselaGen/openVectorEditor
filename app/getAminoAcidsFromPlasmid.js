...
function getAminoAcidsForEntirePlasmid(frame, sequence, forward, circular) {
  ...
  return getAminoAcidsFromSequenceString(sequence, frame);
...
function getAminoAcidsFromSequenceString(sequenceString, frame) {
  ...
  for (var i = 0; i < frame; i++) {
    ...
    res.aminoAcidDataForEachBaseOfDNA.push({
      aminoAcid: getAminoAcidFromSequenceString('xxx'), //fake xxx triplet returns the gap amino acid
      positionInCodon: i,
  ...
  for (var index = 3 + frame; index < sequenceString.length; index += 3) {
    ...
    var aminoAcid = getAminoAcidFromSequenceString(triplet);
    res.aminoAcidDataForEachBaseOfDNA.push({
    ...
    res.aminoAcidStringCorrespondingToEachBaseOfDNA += aminoAcid + aminoAcid + aminoAcid;
