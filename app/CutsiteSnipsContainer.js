...
function getSnipForRow(snipPosition, row, sequenceLength, bpsPerRow, snipStyle, charWidth) {
    ...
    if (row.start <= snipPosition && row.end + 1 >= snipPosition || (row.end === sequenceLength - 1 && row.end < snipPosition)) {
        ...
        var cursorEl = <div className="snip" style={newCursorStyle}/>
        return (cursorEl);
        // onHover={self.onCursorHover}
    }
}

function getSnipConnector(snipRange, row, sequenceLength, bpsPerRow, snipConnectorStyle, charWidth) {
    ...
    return overlaps.map(function(overlap) {
        ...
        var cursorEl = <div className="snipConnector" style={newCursorStyle}/>
        return (cursorEl);
    });
    ...
    render: function() {
        ...
        annotationRanges.forEach(function(annotationRange) {
            ...
            if (areNonNegativeIntegers([downstreamBottomSnip, downstreamTopSnip])) {
                if (topStrand) {
                    newSnip = getSnipForRow(downstreamTopSnip, row, sequenceLength, bpsPerRow, snipStyle, charWidth);
                    if (newSnip) {
                        snips.push(newSnip)
                    }
                } else {
                    newSnip = getSnipForRow(downstreamBottomSnip, row, sequenceLength, bpsPerRow, snipStyle, charWidth);
                    if (newSnip) {
                        snips.push(newSnip)
                    }
                    ...
                    newConnector = getSnipConnector(snipRange, row, sequenceLength, bpsPerRow, snipConnectorStyle, charWidth);
                    snipConnectors.push(newConnector)
                }
            }
            if (areNonNegativeIntegers([upstreamBottomSnip, upstreamTopSnip])) {
                if (topStrand) {
                    newSnip = getSnipForRow(upstreamTopSnip, row, sequenceLength, bpsPerRow, snipStyle, charWidth);
                    if (newSnip) {
                        snips.push(newSnip)
                    }
                } else {
                    newSnip = getSnipForRow(upstreamBottomSnip, row, sequenceLength, bpsPerRow, snipStyle, charWidth);
                    if (newSnip) {
                        snips.push(newSnip)
                    }
